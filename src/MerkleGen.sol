...
library MerkleGen {
        ...
        returns (bytes32[] memory, bool[] memory, bytes32)
    {
        ...
        while (layer.length > 1) {
            parent_layer = computeParentLayer(layer);
        ...
        for (uint256 i = 0; i < layers.length - 1; i++) {
            ...
            layer = layers[i];
            (indices, subproof, source_flags) = proveSingleLayer(layer, indices);
        ...
        bytes32 root = verifyComputeRoot(indexed_leaves, proof_hashes, proof_source_flags);
        ...
        returns (bytes32[] memory, bytes32)
    {
        ...
        bytes32[] memory proof = getProof(leaves, leafIndex);
     ...
     */
    function hashLeafPairs(bytes32 a, bytes32 b) internal pure returns (bytes32 h) {
        if (a < b) {
     ...
     */
    function computeParentLayer(bytes32[] memory layer) internal pure returns (bytes32[] memory) {
        if (layer.length == 1) {
            return layer;
        }
        ...
        for (uint256 i = 0; i < layer.length; i += 2) {
            parent_layer = parent_layer.append(hashLeafPairs(layer[i], layer[i + 1]));
        }
     ...
     * @return Flags indicating the source of each proof hash.
     */
    function proveSingleLayer(bytes32[] memory layer, uint256[] memory indices)
        internal
        pure
        returns (uint256[] memory, bytes32[] memory, bool[] memory)
    {
        ...
        while (j < indices.length) {
            uint256 x = indices[j];
            next_indices = next_indices.append(getParentIndex(x));

            if (((j + 1) < indices.length) && (indices[j + 1] == getSiblingIndex(x))) {
                j += 1;
                source_flags = source_flags.append(SOURCE_FROM_HASHES);
            } else {
                auth_indices = auth_indices.append(getSiblingIndex(x));
     ...
     */
    function helperCount(bool[] memory flags, bool flag) internal pure returns (uint256) {
        uint256 count = 0;
     ...
     */
    function verifyComputeRoot(bytes32[] memory leaves, bytes32[] memory proof_hashes, bool[] memory proof_source_flags)
        internal
        pure
        returns (bytes32)
    {
        ...
        require(
            helperCount(proof_source_flags, SOURCE_FROM_PROOF) == proof_hashes.length,
     ...
     */
    function initTree(bytes32[] memory leaves) internal pure returns (bytes32[] memory) {
        require(leaves.length > 1, "MerkleGen: Leaves should be greater than 1.");
     ...
     */
    function buildTree(bytes32[] memory leaves) internal pure returns (bytes32[] memory) {
        bytes32[] memory tree = initTree(leaves);

        for (uint256 i = tree.length - 1; i > 1; i -= 2) {
            bytes32 left = tree[i - 1];
            bytes32 right = tree[i];
            bytes32 parent = hashLeafPairs(left, right);
